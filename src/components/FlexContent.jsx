...
const FlexContent = ({ifExists, endpoint : {title, heading, text, img, btn,  url}}) => {
  return (
    ...
    lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row-'}`}>
      ...
      lg:max-w-none w-full'>
        <img src={img} alt="img/flex-content" className={`w-auto object-fill transitions-theme
         ${ifExists ? "h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12" : 
         "h-72 lg:h-64 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12"}`}/>
      </div>
    </div>
  )
}
